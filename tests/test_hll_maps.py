...
def test_map_command_exposes_every_tactical_map_as_a_choice() -> None:
...
def test_every_map_choice_has_a_local_tactical_image() -> None:
...
def test_development_placeholder_maps_are_excluded() -> None:
    filenames = set(MAP_IMAGE_FILES.values())

    assert all(f"wdev{letter}.webp" not in filenames for letter in "abcdef")
