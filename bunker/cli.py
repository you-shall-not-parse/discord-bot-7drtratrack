...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--server", nargs=2, action="append", required=True, metavar=("NAME", "DIRECTORY"))
    ...
    for key in ("request_delay", "timeout", "cache_ttl_days", "banned_ttl_days"):
    ...
    sources = [(name.strip(), Path(directory)) for name, directory in args.server]
    for name, directory in sources:
        if not name or not directory.exists():
            parser.error(f"Invalid server name or missing source directory: {directory}")
