...
How to use: Members use the persistent panel in the configured request channel to request a map or temporary admin cam access. Map requests page through a base map and its exact mode and time-of-day variant. Admin cam requests select either Events or Public, ask for a duration from 1 to 168 hours, and use the member's shared T17 mapping. Staff review either request in the approval channel and press **Approve Request** or **Deny**.
