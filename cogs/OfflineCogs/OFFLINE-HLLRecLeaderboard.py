...
DB_FILE = data_path("recleaderboard.db")
...
# ---------------- Cog ----------------
class HLLRecLeaderboard(commands.Cog):
    ...
    async def build_leaderboard_embed(self, monthly: bool = False):
        embed = discord.Embed(
            title="Hell Let Loose Recon Leaderboard" + (" - This Month" if monthly else ""),
    ...
    async def update_leaderboard(self):
        channel = await self._get_channel(LEADERBOARD_CHANNEL_ID)
        if not channel:
            print("HLLRecLeaderboard: LEADERBOARD_CHANNEL_ID not found. Skipping update.")
            return
        ...
        if msg:
            try:
                ...
                print(f"HLLRecLeaderboard: Failed to edit leaderboard message: {e}")
        else:
            try:
                new_msg = await channel.send(embed=embed, view=LeaderboardView(self))
                await self.set_leaderboard_message(new_msg.id)
            except Exception as e:
                print(f"HLLRecLeaderboard: Failed to send leaderboard message: {e}")
    ...
    async def on_ready(self):
        ...
        if not self._db_initialized:
            try:
                ...
                print(f"HLLRecLeaderboard: DB init failed: {e}")
        ...
        if not self._view_registered:
            try:
                ...
                print(f"HLLRecLeaderboard: Failed to register persistent view: {e}")
        ...
        if not self._synced:
            try:
                ...
                print(f"HLLRecLeaderboard: Command sync failed: {e}")
        ...
        if not self._cleanup_started:
            try:
                ...
                print(f"HLLRecLeaderboard: Failed to start cleanup loop: {e}")
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            print(f"HLLRecLeaderboard: on_message proof handling failed: {e}")
    ...
    async def proof_cleanup(self):
        try:
            ...
            print(f"HLLRecLeaderboard: proof cleanup failed: {e}")
...
class SubmissionModal(Modal):
    def __init__(self, cog: HLLRecLeaderboard, stat: str, user: discord.abc.User):
...
class LeaderboardView(View):
    def __init__(self, cog: HLLRecLeaderboard):
...
async def setup(bot: commands.Bot):
    await bot.add_cog(HLLRecLeaderboard(bot))
