...
BROADCAST_NO_VOTES = "No votes, same map again..."
...
def rcon_set_map_rotation(map_ids: list[str]):
...
class MapVote(commands.Cog):
    ...
    # --------------------------------------------------
    # PER-PLAYER BROADCAST USING get_players + message_player
    # --------------------------------------------------
    ...
    async def start_vote(self, gs):
        ...
        # Build option list (NOTE: may still include current map if IDs differ)
    ...
    async def end_vote_and_queue(self):
        ...
        if not winner_id:
            # No votes → set rotation to current map only
            if not self.state.match_map_id:
                await channel.send("No votes — and current map ID unknown, cannot set rotation.")
                print("[MapVote] No votes and match_map_id is None.")
                return

            res = rcon_set_map_rotation([self.state.match_map_id])
            await self.broadcast_to_all(BROADCAST_NO_VOTES)
            await channel.send(
                "No votes — map rotation continues with the **current map only**.\n"
                f"CRCON Response:\n```{res}```"
            )
            print("[MapVote] Vote ended with no votes; rotation set to current map only.")
            return

        pretty = next((p for p, mid in MAPS.items() if mid == winner_id), winner_id)

        # Winner → rotation becomes [winner_id]
        result = rcon_set_map_rotation([winner_id])

        await self.broadcast_to_all(f"{pretty} has won the vote!")
        await channel.send(
            f"🏆 **Winner: {pretty}**\n"
            f"Map rotation set to this map only until changed.\n"
            f"CRCON Response:\n```{result}```"
        )
