...
class SubmissionModal(Modal):
    def __init__(self, cog, stat, user):
        super().__init__(title=f"Submit {stat}")
        self.cog = cog
        self.stat = stat
        self.user = user
        self.value_input = InputText(label="Enter your score", placeholder="e.g. 10")
...
class StatSelect(Select):
    def __init__(self, cog):
        ...
        super().__init__(placeholder="Select stat to submit", options=options)
        self.cog = cog
